...
async def list_my_tenants(
    ...
    (`get_current_principal`/`resolve_principal` can't be used here: they
    require already having resolved to exactly one Principal, which is
    ...
    where they already have a Principal (matched by `sub`, the same
    lookup `resolve_principal` uses), never more -- this must not become a
