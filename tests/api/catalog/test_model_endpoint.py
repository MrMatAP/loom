...
@pytest.mark.asyncio
async def test_model_endpoint_full_lifecycle(api_client, fake_principal):
    ...
    get_resp = await api_client.get(
        f'/api/v1/tenants/{tenant_id}/model-endpoints/{entity_id}'
    )
    ...
    transition_resp = await api_client.post(
        f'/api/v1/tenants/{tenant_id}/model-endpoints/{entity_id}/versions/2/transitions',
...
@pytest.mark.asyncio
async def test_model_endpoint_openai_compatible_requires_base_url(
    api_client, fake_principal
):
    resp = await api_client.post(
        f'/api/v1/tenants/{fake_principal.tenant_id}/model-endpoints',
        json={
...
@pytest.mark.asyncio
async def test_model_endpoint_not_found_returns_404(api_client, fake_principal):
...
@pytest.mark.asyncio
async def test_agent_can_bind_to_a_model_endpoint(api_client, fake_principal):
    tenant_id = fake_principal.tenant_id
    ...
    lookup_resp = await api_client.get(
        f'/api/v1/tenants/{tenant_id}/model-endpoints/'
        f'{agent_resp.json()["model_binding_id"]}'
    )
...
@pytest.mark.asyncio
async def test_agent_model_binding_floats_to_the_new_current_version(
    api_client, fake_principal
):
    tenant_id = fake_principal.tenant_id
    endpoint_resp = await api_client.post(
    ...
    version_resp = await api_client.post(
        f'/api/v1/tenants/{tenant_id}/model-endpoints/{endpoint_entity_id}/versions',
        json={
            'name': 'Self-Hosted Llama v2',
            'protocol': 'openai_compatible',
            'base_url': 'https://llm.internal.example/v2',
    ...
    lookup_resp = await api_client.get(
        f'/api/v1/tenants/{tenant_id}/model-endpoints/{endpoint_entity_id}'
    )
...
async def test_agent_rejects_model_binding_from_another_tenant(
    api_client, fake_principal, async_session_factory
):
    tenant_id = fake_principal.tenant_id
